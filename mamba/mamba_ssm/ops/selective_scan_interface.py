...
def _as_contiguous_last(tensor):
    if tensor is None:
        return None
    if tensor.stride(-1) != 1:
        ...
        if tensor.stride(-1) != 1:
            if tensor.shape[-1] == 1:
                ...
                strides[-1] = 1
                tensor = tensor.as_strided(tensor.shape, tuple(strides))
            else:
...
class SelectiveScanFn(torch.autograd.Function):
    ...
    def forward(ctx, u, delta, A, B, C, D=None, z=None, delta_bias=None, delta_softplus=False,
        ...
        if D is not None:
            D = D.contiguous()
        B = _as_contiguous_last(B)
        C = _as_contiguous_last(C)
...
def selective_scan_fn(u, delta, A, B, C, D=None, z=None, delta_bias=None, delta_softplus=False,
    ...
    try:
        ...
        # 某些 layout（尤其是 transpose/flip 后）会让 B/C 的 stride 不满足 CUDA kernel 的硬性要求。
        # 这里自动回退到纯 PyTorch reference 实现（更慢，但可训练且不依赖 stride 限制）。
        msg = str(e)
        if "Expected B.stride(-1) == 1" in msg or "stride(-1)" in msg:
...
class MambaInnerFnNoOutProj(torch.autograd.Function):
    ...
    def forward(ctx, xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
        ...
        if D is not None:
            D = D.contiguous()
        # 最终保险：强制 B/C 连续，避免 kernel 对 stride 的硬性要求触发报错
        B = _as_contiguous_last(B)
        C = _as_contiguous_last(C)
...
#         # ===【修改开始】添加这几行检查 ===
#         if conv1d_out.stride(-1) != 1: conv1d_out = conv1d_out.contiguous()
#         if delta.stride(-1) != 1: delta = delta.contiguous()
#         if B.stride(-1) != 1: B = B.contiguous()  # <--- 这就是报错的元凶
...
# ==========================================
        ...
        # 2. 智能修正 B 的形状 (关键步骤！)
        if B is not None:
            # 获取序列长度 L (从 conv1d_out 获取，形状通常是 [Batch, Dim, Length])
            L = conv1d_out.shape[-1]
            
            # 如果 B 的最后一维不是 L，说明它是 [Batch, Length, Dim]，需要转置！
            if B.shape[-1] != L:
                # print(f"DEBUG: Fixing B shape from {B.shape} -> transpose", flush=True)
                if B.dim() == 3: # [B, L, D] -> [B, D, L]
                    B = B.transpose(1, 2)
                elif B.dim() == 4: # [B, 1, L, D] -> [B, 1, D, L]
                    B = B.transpose(2, 3)
        ...
        if C is not None:
            L = conv1d_out.shape[-1]
            if C.shape[-1] != L:
                # print(f"DEBUG: Fixing C shape from {C.shape} -> transpose", flush=True)
                if C.dim() == 3:
                    C = C.transpose(1, 2)
                elif C.dim() == 4:
                    C = C.transpose(2, 3)
...
class BiMambaInnerFn(torch.autograd.Function):
    ...
    def forward(ctx, xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
        ...
        out_b, scan_intermediates_b, out_z_b = selective_scan_cuda.fwd(
            ...
            delta.flip([-1]),
            A_b,
            _as_contiguous_last(B.flip([-1])),
            _as_contiguous_last(C.flip([-1])),
            D,
            z.flip([-1]),
            delta_bias,
            delta_softplus,
